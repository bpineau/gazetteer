...
var sourceRenderers = map[string]sourceRenderer{
	ademe.Name:        typed("no DPE found", renderAdeme),
	anct.Name:         typed("no ANCT programme", renderAnct),
	bdnb.Name:         typed("no building data", renderBDNB),
	bpe.Name:          typed("no facilities indexed", renderBPE),
	cadastre.Name:     renderCadastre, // bespoke guard: also empty when len(Parcels)==0
	carteloyers.Name:  typed("no rent reading", renderCarteloyers),
	cartofriches.Name: typed("no referenced sites", renderCartofriches),
	catnat.Name:       typed("no CatNat decree on record", renderCatnat),
	cdsr.Name:         typed("no distressed copro within range", renderCDSR),
	chomage.Name:      typed("no unemployment reading", renderChomage),
	delinquance.Name:  typed("absent from SSMSI dataset", renderDelinquance),
	dpedist.Name:      typed("no DPE in this commune", renderDPEDist),
	dvf.Name:          typed("no comparable transactions", renderDVF),
	dvfagg.Name:       typed("no commune-level DVF aggregate", renderDVFAgg),
	education.Name:    typed("no schools registered", renderEducation),
	encadrement.Name:  typed("outside any encadrement zone", renderEncadrement),
	filoiris.Name:     typed("no IRIS-level income (outside the ≥5000-hab perimeter)", renderFiloIris),
	filosofi.Name:     typed("absent from Filosofi", renderFilosofi),
	georisques.Name:   typed("no risk data for these coords", renderGeorisques),
	gpe.Name:          typed("no future GPE station within range", renderGPE),
	iris.Name:         typed("outside covered IRIS perimeter", renderIRIS),
	ipsecoles.Name:    typed("no école primaire IPS on record", renderIPSEcoles),
	locservice.Name:   typed("no tension reading", renderLocservice),
	logiris.Name:      typed("no IRIS housing data (outside IDF)", renderLogiris),
	lovac.Name:        typed("absent from LOVAC", renderLovac),
	nuisances.Name:    typed("outside the nuisance grid", renderNuisances),
	oll.Name:          typed("no observed-rent cell (Paris intra-muros is out of OLL scope)", renderOLL),
	gzosm.Name:        typed("no transit in range", renderOSM),
	qpv.Name:          typed("address not in a QPV", renderQPV),
	rnc.Name:          typed("no copropriété matched in the RNC", renderRNC),
	rpls.Name:         typed("absent from the SRU inventory", renderRPLS),
	sensible.Name:     typed("no sensitive-zone perimeter here", renderSensible),
	sitadel.Name:      typed("no construction data for this commune", renderSitadel),
	taxefonciere.Name: typed("no estimate", renderTaxeFonciere),
	vacance.Name:      typed("absent from the census vacancy dataset", renderVacance),
	zonageabc.Name:    typed("absent from zonage ABC dataset", renderZonageABC),
