...
func runSources(ctx context.Context, args []string, w streams) error {
	if len(args) == 0 {
		printSourcesUsage(w.err)
		return errUsage
	}
	sub, rest := args[0], args[1:]
	switch sub {
	case "list":
		return runSourcesList(ctx, rest, w)
	case "doc":
		return runSourcesDoc(ctx, rest, w)
	case "catalog":
		return runSourcesCatalog(rest, w)
	case "dimensions":
		return runSourcesDimensions(rest, w)
	case "-h", "--help", "help":
		printSourcesUsage(w.out)
		return nil
	default:
		fmt.Fprintf(w.err, "unknown sources sub-command %q\n\n", sub)
		printSourcesUsage(w.err)
		return errUsage
	}
}
...
// not the Source.
func runSourcesList(_ context.Context, args []string, w streams) error {
	...
	for _, name := range registered {
		f, ok := byName[name]
		if !ok {
			fmt.Fprintf(w.out, "%-14s  (no CLI factory)\n", name)
			continue
		}
		src, err := f.Build(deps)
		if err != nil {
			fmt.Fprintf(w.out, "%-14s  build error: %v\n", name, err)
			continue
		}
		dflt := ""
		if !f.Default {
...
// source.
func runSourcesDoc(_ context.Context, args []string, w streams) error {
