...
func (s *Source) Query(ctx context.Context, l gazetteer.Listing) (any, error) {
	...
	if idx == nil {
		loaded, err := Load()
		if err != nil {
			return nil, fmt.Errorf("zonetendue: %w: load dataset: %w", gazetteer.ErrUpstreamPermanent, err)
		}
		idx = loaded
	}

	// Paris / Lyon / Marseille arrondissement codes inherit the
	// parent commune's classification — the dataset only stores the
	// 75056 / 69123 / 13055 rows.
