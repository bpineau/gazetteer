...
type Index struct {
	Meta      Meta
	geo       *geoindex.Index[hit]
	byCommune map[string]Entry
}

// hit is the per-polygon payload carried by the geoindex: the QPV identity
// returned by a point-in-polygon or nearest resolve.
type hit struct {
	Code  string
	Label string
}
...
func NewIndexForTest(feats []FeatureForTest) *Index {
	...
	gfeats := make([]geoindex.Feature[hit], 0, len(feats))
	for _, f := range feats {
		gfeats = append(gfeats, geoindex.NewFeature(hit{Code: f.Code, Label: f.Label}, f.Polygons))
...
func parseIndex(r io.Reader) (*Index, error) {
	...
	gfeats := make([]geoindex.Feature[hit], 0, len(p.QPVs))
	for _, r := range p.QPVs {
		gfeats = append(gfeats, geoindex.NewFeature(hit{Code: r.Code, Label: r.Label}, r.Polygons.MultiPolygon()))
