...
const rawName = "vacance_logements.raw.zip"
...
// transform rebuilds the processed vacance_logements artifact from the
...
func transform(_ context.Context, raw dataset.RawSet, dst io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("vacance_logements: read raw zip: %w", err)
	}
	zr, err := zip.NewReader(bytes.NewReader(blob), int64(len(blob)))
	if err != nil {
		return fmt.Errorf("vacance_logements: open raw zip: %w", err)
	}
	...
	mrc, err := member.Open()
	if err != nil {
		return fmt.Errorf("vacance_logements: open zip member %q: %w", member.Name, err)
	}
	...
	header, err := cr.Read()
	if err != nil {
		return fmt.Errorf("vacance_logements: read header: %w", err)
	}
	...
	if insee < 0 || logC < 0 || vacC < 0 || rpC < 0 || rsecC < 0 {
		return fmt.Errorf("vacance_logements: header missing required columns: %v", header)
	}
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("vacance_logements: read row: %w", err)
		}
	...
	if len(idx.Communes) == 0 {
		return errors.New("vacance_logements: transform produced no communes")
	}
	...
	if err := json.NewEncoder(zw).Encode(idx); err != nil {
		_ = zw.Close()
		return fmt.Errorf("vacance_logements: encode json: %w", err)
	}
	return zw.Close()
}
...
func validate(r io.Reader) error {
	...
	if idx.Count() == 0 {
		return errors.New("vacance_logements: validated artifact has no communes")
	}
	return nil
}
...
func dataCSVMember(zr *zip.Reader) (*zip.File, error) {
	...
	return nil, errors.New("vacance_logements: data CSV not found in raw zip")
}
