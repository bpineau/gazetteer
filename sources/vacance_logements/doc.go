// Package vacance_logements is a gazetteer.Source that returns the
...
// under `data/vacance_logements_communes.json.gz`.
//
...
// the build script (`/tmp/gazetteer-data/build_vacance_logements.py`)
...
// payload:
//
//	src := vacance_logements.NewSource(vacance_logements.Options{})
//	data, err := src.Query(ctx, gazetteer.Listing{INSEE: "42218"}) // Saint-Étienne
//	if err != nil { log.Fatal(err) }
//	r := data.(*vacance_logements.Result)
//	if r.IsEmpty() {
