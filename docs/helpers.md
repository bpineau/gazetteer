...
|---|---|---|
| [`helpers/httpx`](../helpers/httpx) | A polite production HTTP client: per-host token-bucket rate limits, Retry-After-honouring backoff, an on-disk ETag/Last-Modified-revalidating response cache whose misses are single-flighted per cache key, atomic snapshot downloads, a shared User-Agent. The cache never evicts on its own: `(*Client).PruneCache(PruneOptions{MaxAge, MaxBytes})` is explicit and never automatic. | Any scraper or open-data client. Start per-host limits from `factory.HostRateLimits()` (production-tuned values for BAN, data.gouv.fr, ADEME, …) and extend. |
...
| [`helpers/kvcache`](../helpers/kvcache) (+ `kvcache/memcache`, `kvcache/kvcachetest`) | A minimal persistent key-value cache **interface** with TTL semantics, a bounded in-memory implementation (entry ceiling via `memcache.WithMaxEntries`, expired rows swept before the least-recently-used one is evicted), and a conformance test suite for your own backend (consumers run SQLite/bun adapters through it). See [caching.md](caching.md). | You need "cache this geocode/section list across process restarts" with a backend you own. |
...
| Package | What it gives you | Reach for it when |
|---|---|---|
| [`helpers/banx`](../helpers/banx) | A BAN (Base Adresse Nationale) geocoding client: forward + reverse, score filtering, a cache decorator with département-coherence guards against homonym drift, the INSEE-resolution cascade (`INSEEResolver`), the match GRANULARITY (`Precision`: housenumber / street / locality / municipality, with `ResolveLatLonAt` to refuse anything coarser than a caller needs - BAN answers a nonexistent address with its commune's centre), zip/département reasoning (`ZipsShareDepartment`), and `NewDefaultGeocoder` - the exact cached+guarded stack the factory wires. `banx/maaddr` canonicalizes addresses for matching. | Anything that turns French free-text addresses into coordinates/INSEE codes. |
| [`helpers/communes`](../helpers/communes) | The embedded table of ~35k French communes: INSEE↔name/zip lookup, centroids, neighbors-within-radius, same-department, Paris/Lyon/Marseille arrondissement folding (`FoldArrondissement`, `ArrondissementParents`, and its mirror `IsArrondissementParent` for the datasets keyed the other way, like geo-dvf), zip→département (`DeptFromZip`), the INSEE↔postcode coherence test `ZipMatchesINSEE` (23 communes are served by a postcode of a neighbouring département, so comparing prefixes answers a different question), and the fully offline `ResolveINSEE(city, zip)` with the PLM postal rules. Note: the table is an embedded snapshot - it does **not** participate in `gazetteer refresh` and ages with the module version. | Any commune-level lookup, INSEE resolution without network I/O, or "which communes are near X". |
