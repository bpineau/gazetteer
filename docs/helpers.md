...
| [`helpers/geopoly`](../helpers/geopoly) | The polygon kernel: even-odd point-in-polygon (`Covers`), distance to the nearest boundary EDGE (`BoundaryDistanceM`, never the nearest vertex), bounding boxes, centroids, equirectangular areas in m². | Point-in-area tests against GeoJSON-shaped geometry. |
